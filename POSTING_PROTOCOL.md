...
The BetLegendPicks homepage Latest Blog Picks cards and Pick Archive cards are
Google Sheet pick surfaces only. Their source of truth is `homepage-picks-data.js`.

Rules:
- Only real Google Sheet pick pages belong in homepage cards and Pick Archive cards.
- Do not insert preview pages, featured-game pages, sport hubs, monthly archive/calendar pages, records pages, or guide/news pages into those cards.
- Normal cadence is one or two Google Sheet picks per day, newest first.
- `homepage-picks-data.js` controls the visible card sequence; new sheet picks must be added at the top with exact date, URL, image, sport, and result when graded.
- Before publishing, run `python scripts/validate_homepage_pick_image_uniqueness.py`.
...
calendar article, and pick. Nothing is complete if it looks generic, lacks
graphics, uses the wrong format, hides older posts from the calendar, or
contains any unverified claim.

### NON-NEGOTIABLE REQUIREMENTS

1. **Use the correct historical BetLegend format.**
   - Match the strong old/Claude-style sports pages already in the repo.
   - Use the established sports article structure, including matchup blocks,
     team logos/crests/badges, sport calendar sidebar, mobile archive selector,
     dated metadata, and the correct sport calendar script.
   - Do not ship bare placeholder cards, generic dark panels, or a new template
     that does not match the existing BetLegend archive/page style.

2. **Graphics are required, not optional.**
   - Every matchup section must include real team logos/crests when verified.
   - If a real logo cannot be verified, use a clean labeled badge and do not
     represent it as an official mark.
   - If a page includes a featured image or graphic, the article must mention
     what the visual is showing and connect it to the matchup/slate.

3. **No fake information, ever.**
   - Records, standings, team form, stats, injuries, player availability,
     betting lines, totals, moneylines, schedules, venues, networks, trends,
     splits, ATS data, and market movement must be verified from current
     sources or omitted.
   - Never invent a plausible stat, trend, quote, injury note, betting line, or
     schedule detail.
   - Do not write claims like "hot", "struggling", "public side", "sharp
     money", "trend points", or "line movement" unless the evidence has been
     checked.

4. **No AI slop or filler.**
   - Every section must be specific to the actual teams, players, matchup,
     schedule spot, market context, and verified statistical picture.
   - Generic paragraphs that could fit any game are not acceptable.
   - Banned placeholders include "coming soon", "TBD", "analysis pending",
     "check back", "generic preview", and empty matchup commentary.

5. **Calendar safety is required.**
...
screenshots, localhost, source checks, build/deploy success, and HTTP checks do
not satisfy this requirement. Confirm the title/header is centered and
...
- `phillies-pirates-first-five-under-4-5-wheeler-skenes-pnc.html`
- `rangers-team-total-under-4-5-eovaldi-lambert-daikin-park.html`
